            ...
            ?   <> { this.props.store.comments.map((comment, k) => <React.Fragment key={k}>
                    <div className="comment">
                        <div className="f6">
                            ...
                            <div className="d-inline-block"><span>Points: {comment.points} {this.props.store.authStore.isAuth && !comment.isDeleted && <button className="btn-icon ml-n2" onClick={() => this.props.store.togglePoints(comment.id)}>{!comment.isPointed ? <div className="gg-chevron-up"></div> : <div className="gg-chevron-up text-blue"></div>}</button>}</span></div>
