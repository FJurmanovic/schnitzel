...
const Logo = (props) => {
    ...
    return (
        <img
            src={`https://api.jurmanovic.com/schnitzel/api/logos/${logos[current]}.svg`}
            className="logo"
        ></img>
    );
};
...
const NotLoggedLink = () => (
    <>
        <div className="header-item mr-5">
            <Link to="/explore" className="btn btn-white btn-rounder">
                Explore
            </Link>
        </div>
...
const LoggedLink = observer(({ userData, toggleMenu }) => (
    <>
        <div className="header-item">
            <SearchComponent store={searchStore} />
        </div>
        <div className="header-item mr-5 explore-btn">
            <Link
                to="/explore"
                className="btn btn-white btn-rounder explore-btn"
            >
                Explore
            </Link>
        </div>
        <div className="header-item">
            <details className="header-dropdown" id="profile">
                <summary
                    className="btn btn-default px-7 header-button"
                    onClick={(event) => {
                ...
                >
                    ...
                    <div className="sml-photo mx-auto text-center">
                        {userData.hasPhoto ? (
                            <img src={userData.url} className="card-img-top" />
                        ) : (
                            <img
                                src="https://storage.googleapis.com/schnitzel/default.jpg"
                ...
                <ul className="header-dropdown-menu dropdown-menu-dark">
                    <Link
                        to="/explore"
                        className="dropdown-item explore-btn-small"
                    >
                        Explore
                    </Link>
        ...
        return (
            <>
                <header className="header border-bottom border-black p-5 f4">
                    ...
                    {this.props.AuthStore.isAuth ? (
                        <LoggedLink
                            userData={this.props.AuthStore.userData}
                            toggleMenu={this.props.AuthStore.toggleMenu}
                        />
                    ) : (
                        <NotLoggedLink />
                    )}
                </header>
                {this.props.AuthStore.isAuth && this.props.AuthStore.menuShow && (
